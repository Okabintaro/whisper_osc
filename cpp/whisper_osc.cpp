...
bool audio_sdl_init(const int capture_id) {
  ...
  if (capture_id >= 0) {
    ...
    g_dev_id_in = SDL_OpenAudioDevice(
...
int main(int argc, char **argv) {
  ...
  // print some info about the processing
  {
    ...
    fprintf(stderr,
            "%s: processing %d samples (step = %.1f sec / len = %.1f sec), %d "
            "threads, lang = %s, task = %s, timestamps = %d ...\n",
            __func__, n_samples, float(n_samples) / WHISPER_SAMPLE_RATE,
            float(n_samples_len) / WHISPER_SAMPLE_RATE, params.n_threads,
            params.language.c_str(),
  ...
  while (is_running) {
    ...
    // process new audio
    if (n_iter > 0 &&
        SDL_GetQueuedAudioSize(g_dev_id_in) > 2 * n_samples * sizeof(float)) {
      fprintf(stderr,
    ...
    SDL_DequeueAudio(g_dev_id_in, pcmf32.data() + n_samples_take,
        ...
        for (int i = 0; i < n_segments; ++i) {
          ...
          if (params.no_timestamps) {
            ...
            if (strcmp(text, " [BLANK_AUDIO]") != 0 &&
                strcmp(text, " (buzzing)") != 0 &&
                strcmp(text, last_string) != 0) {
              printf("Sending: '%s'}\n", text);
              memset(&addr, 0, sizeof(addr));
              addr.sin_family = AF_INET;
              addr.sin_port = htons(9000);
              addr.sin_addr.s_addr = INADDR_ANY;
              strncpy(last_string, text, 256);

              // write the OSC packet to the buffer
              // returns the number of bytes written to the buffer, negative on
              // error note that tosc_write will clear the entire buffer before
              // writing to it
              int len = tosc_writeMessage(
                  buffer, sizeof(buffer),
                  "/chatbox/input", // the address
                  "sTT",            // the format; 'f':32-bit float,
